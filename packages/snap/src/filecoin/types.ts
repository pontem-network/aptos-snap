import {Message, SignedMessage, Transaction} from "@pontem/aptosnap-types";
import {AptosSigningMessageRequest, AptosSubmitTransactionResponse, AptosSubmitTransactionRequest} from "../interfaces";
...
export interface AptosRPCApi {
  walletBalance(address: string): Promise<string>;
  getSequenceNumber(address: string): Promise<string>;
  submitTransaction(tx: AptosSubmitTransactionRequest): Promise<AptosSubmitTransactionResponse>;
  getSigningMessage(message: AptosSigningMessageRequest): Promise<string>;
